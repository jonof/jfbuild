...
void loadwadheader(void) {
  ...
  numwads = ((int)tempbuf[4]) + (((int)tempbuf[5]) << 8) +
            (((int)tempbuf[6]) << 16) + (((int)tempbuf[7]) << 24);
  i = ((int)tempbuf[8]) + (((int)tempbuf[9]) << 8) +
      (((int)tempbuf[10]) << 16) + (((int)tempbuf[11]) << 24);
  Blseek(fil1, i, BSEEK_SET);
  Bread(fil1, &tempbuf[0], numwads * 16);
  j = 0;
  for (i = 0; i < numwads; i++) {
    wadplc[i] = ((int)tempbuf[j]) + (((int)tempbuf[j + 1]) << 8) +
                (((int)tempbuf[j + 2]) << 16) + (((int)tempbuf[j + 3]) << 24);
    j += 4;
    wadlen[i] = ((int)tempbuf[j]) + (((int)tempbuf[j + 1]) << 8) +
                (((int)tempbuf[j + 2]) << 16) + (((int)tempbuf[j + 3]) << 24);
    j += 4;
    wadata[i][0] = tempbuf[j + 0];
    wadata[i][1] = tempbuf[j + 1];
    wadata[i][2] = tempbuf[j + 2];
    wadata[i][3] = tempbuf[j + 3];
    wadata[i][4] = tempbuf[j + 4];
    wadata[i][5] = tempbuf[j + 5];
    wadata[i][6] = tempbuf[j + 6];
    wadata[i][7] = tempbuf[j + 7];
    j += 8;
  }
}
...
void convpalette(void) {
  ...
  printf("Going for PLAYPAL...\n");

  i = 0;
  while (Bstrncasecmp(wadata[i], "PLAYPAL", 7) != 0) {
    // printf("%s", wadata[i]);
    i++;
  }
  ...
  printf("Going for COLORMAP...\n");
  i = 0;
  while (Bstrncasecmp(wadata[i], "COLORMAP", 8) != 0)
    i++;
  Blseek(fil1, wadplc[i], BSEEK_SET);
  Bread(fil1, palookup, 8192);

  printf("Going to save PALETTE.DAT...\n");
  if ((fil3 = Bopen("palette.dat", BO_BINARY | BO_TRUNC | BO_CREAT | BO_WRONLY,
                    BS_IREAD | BS_IWRITE)) == -1) {
...
void savenames(void) {
  ...
  if ((fil3 = Bopen("names.h", BO_BINARY | BO_TRUNC | BO_CREAT | BO_WRONLY,
                    BS_IREAD | BS_IWRITE)) == -1)
    return;

  strcpy(buffer, "//Be careful when changing this file - it is parsed by "
                 "Editart and Build.");
  ...
  for (i = 0; i < numwads; i++)
    if (wadata[i][0] != 0) {
      ...
      while ((j < 16) && (wadata[i][j - 8] != 0)) {
        buffer[j] = wadata[i][j - 8];
        j++;
      }
      buffer[j++] = 32;

      if (i >= 10000)
        buffer[j++] = ((i / 10000) % 10) + 48;
      if (i >= 1000)
        buffer[j++] = ((i / 1000) % 10) + 48;
      if (i >= 100)
        buffer[j++] = ((i / 100) % 10) + 48;
      if (i >= 10)
        buffer[j++] = ((i / 10) % 10) + 48;
      buffer[j++] = (i % 10) + 48;
...
void showart(char *part) {
  ...
  if ((Bstrncasecmp(part, "L_START", 7) == 0) ||
      (Bstrncasecmp(part, "S_START", 7) == 0) ||
      (Bstrncasecmp(part, "P_START", 7) == 0)) {
    if (Bstrncasecmp(part, "L_START", 7) == 0)
      z = 462;
    else {
      z = 0;
      while (Bstrncasecmp(wadata[z], part, 7) != 0)
        z++;
      z++;
    }
    ...
    do {
      ...
      if (Bstrncasecmp(wadata[z], "P1_START", 8) == 0)
        z++;
      if (Bstrncasecmp(wadata[z], "P1_END", 6) == 0)
        z++;
      if (Bstrncasecmp(wadata[z], "P2_START", 8) == 0)
        z++;
      if (Bstrncasecmp(wadata[z], "S_START", 7) == 0)
        break;
      if (Bstrncasecmp(wadata[z], "S_END", 5) == 0)
        break;
      if (Bstrncasecmp(wadata[z], "P_END", 5) == 0)
        break;

      if (curplc != wadplc[z])
        Blseek(fil1, wadplc[z], BSEEK_SET);
      read(fil1, &tempbuf[0], wadlen[z]);
      curplc = wadplc[z] + wadlen[z];

      xsiz = (int)tempbuf[0] + (((int)tempbuf[1]) << 8);
      ysiz = (int)tempbuf[2] + (((int)tempbuf[3]) << 8);
      if ((xsiz <= 0) || (ysiz <= 0) || (xsiz > 320) || (ysiz > 200))
        goto skipit;
      i = 8;
      for (zx = 0; zx < xsiz; zx++) {

        xoffses[zx] = ((int)tempbuf[i]) + (((int)tempbuf[i + 1]) << 8) +
                      (((int)tempbuf[i + 2]) << 16) +
                      (((int)tempbuf[i + 3]) << 24);
        i += 4;
      }
    ...
    while (1) {
      if (Bstrncasecmp(wadata[z], "F1_START", 8) == 0)
        z++;
      if (Bstrncasecmp(wadata[z], "F1_END", 6) == 0)
        z++;
      if (Bstrncasecmp(wadata[z], "F_END", 5) == 0)
        break;
...
int main(int argc, char **argv) {
  ...
  printf("Wad2Art!                                       Copyright 1995 by Ken "
         "Silverman\n");

  if (argc != 2) {
    printf("Command line parameters: Wad2Art [Doom IWAD file]\n");
    printf("   Creates TILES000.ART, PALETTE.DAT, and NAMES.H in current "
           "directory.\n");
  ...
  if ((fil2 = Bopen("tiles000.art", BO_BINARY | BO_TRUNC | BO_CREAT | BO_WRONLY,
                    BS_IREAD | BS_IWRITE)) == -1) {
