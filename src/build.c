...
void editinput(void)
{
	...
	{
	ldiv_t ld;
	ld = ldiv((long)((double)mousx*msens), (2<<16)); mousx = ld.quot; mousexsurp = ld.rem;
	ld = ldiv((long)((double)mousy*msens), (2<<16)); mousy = ld.quot; mouseysurp = ld.rem;
	}
	searchx += mousx;
	searchy += mousy;
...
long menuselect(void)
{
	...
	do {
		...
		if (ch == 9) {
			if ((currentlist == 0 && finddirs) || finddirs)
