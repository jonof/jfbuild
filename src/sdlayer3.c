...
//
// Use SDL3 from http://www.libsdl.org
...
static struct keytranslate keytranslation[SDL_SCANCODE_COUNT];
...
#endif
		if (SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, name, buf, sdl_window)) {
			rv = 1;
			break;
		}
...
#endif
		if (SDL_ShowMessageBox(&msgbox, &rv)) {
			rv = (rv == 1);
			break;
		}
...
int initsystem(void)
{
	...
	buildprintf("SDL3 system interface "
...
int initinput(void)
{
	...
	if (!SDL_InitSubSystem(SDL_INIT_GAMEPAD)) {
		...
		if (dbuf) {
			...
			kclose(fh);
			if (flen >= 0) {
				ios = SDL_IOFromConstMem(dbuf, flen);
			}
		}
		if (ios) {
			i = SDL_AddGamepadMappingsFromIO(ios, false);
		...
		if (!SDL_HasJoystick()) {
			...
			for (i = 0; joys && i < numjoysticks; i++) {
				if (SDL_IsGamepad(i)) {
					buildprintf("  - %s\n", SDL_GetGamepadNameForID(joys[i]));
					if (!controller) {
						controller = SDL_OpenGamepad(joys[i]);
					}
				}
			}
			if (controller) {
				...
				inputdevices |= 4;
				joynumaxes    = min(Barraylen(joyaxis), SDL_GAMEPAD_AXIS_COUNT);
				joynumbuttons = SDL_GAMEPAD_BUTTON_COUNT;
			} else {
...
const char *getjoyname(int what, int num)
{
	switch (what) {
		case 0: // axis
			return SDL_GetGamepadStringForAxis(num);
		case 1: // button
			return SDL_GetGamepadStringForButton(num);
		default:
			return NULL;
	}
}
...
void grabmouse(int a)
{
	if (appactive && moustat) {
		if (a != mouseacquired) {
			SDL_SetWindowRelativeMouseMode(sdl_window, !!a);
			mouseacquired = a;
		}
	} else {
		mouseacquired = a;
...
#define CHECKLE(w,h) if ((w <= maxx) && (h <= maxy))
	...
	desktop = SDL_GetDesktopDisplayMode(displays[0]);
	maxx = desktop->w;
	maxy = desktop->h;
	...
	if (!glunavailable) {
		...
		modes = SDL_GetFullscreenDisplayModes(displays[0], NULL);
		for (int j = 0; modes[j]; j++) {
			if ((modes[j]->w > MAXXDIM) || (modes[j]->h > MAXYDIM)) continue;
			if (SDL_BITSPERPIXEL(modes[j]->format) < 8) continue;
			ADDMODE(modes[j]->w, modes[j]->h, SDL_BITSPERPIXEL(modes[j]->format), 1)
		}
		SDL_free(modes);
	}
#endif
...
int setvideomode(int x, int y, int c, int fs)
{
	...
	do {
		...
		sdl_window = SDL_CreateWindow(wintitle, winx, winy, flags);
		if (!sdl_window) {
		...
		if (glunavailable) {
			...
			if (usesdlrenderer) {
				...
				sdl_texture = SDL_CreateTexture(sdl_renderer,
					B_LITTLE_ENDIAN ? SDL_PIXELFORMAT_XBGR8888 : SDL_PIXELFORMAT_RGBX8888,
				...
				// 8-bit software with no GL shader blitting goes via the SDL rendering apparatus.
				sdl_surface = SDL_CreateSurface(x, y,
					B_LITTLE_ENDIAN ? SDL_PIXELFORMAT_XBGR8888 : SDL_PIXELFORMAT_RGBX8888);
				if (!sdl_surface) {
			...
			if (!sdl_glcontext) {
				...
				SDL_GetWindowSizeInPixels(sdl_window, &winx, &winy);
...
#if USE_OPENGL
	if (sdl_glcontext) {
		if (!SDL_GL_SetSwapInterval(glswapinterval)) {
...
void showframe(void)
{
	...
	if (usesdlrenderer) {
		SDL_GetRenderOutputSize(sdl_renderer, &rendx, &rendy);
	} else {
	...
	if (usesdlrenderer) {
		SDL_FRect fdestrect;
		SDL_RectToFRect(&destrect, &fdestrect);
		SDL_RenderClear(sdl_renderer);
		if (!SDL_RenderTexture(sdl_renderer, sdl_texture, NULL, &fdestrect)) {
			debugprintf("Could not copy render texture: %s\n", SDL_GetError());
		}
		SDL_RenderPresent(sdl_renderer);
	} else {
		SDL_FillSurfaceRect(winsurface, NULL, SDL_MapRGB(SDL_GetPixelFormatDetails(winsurface->format), NULL, 0, 0, 0));
		if (!SDL_BlitSurfaceScaled(sdl_surface, NULL, winsurface, &destrect, SDL_SCALEMODE_NEAREST)) {
...
static SDL_Surface * loadappicon(void)
{
	...
	surf = SDL_CreateSurfaceFrom(sdlappicon.width, sdlappicon.height, SDL_PIXELFORMAT_ABGR8888,
			(void*)sdlappicon.pixels, sdlappicon.width*4);
	...
	while (SDL_PollEvent(&ev)) {
		switch (ev.type) {
			...
			case SDL_EVENT_KEY_DOWN:
				...
				if (ev.key.key == SDLK_DELETE && ev.key.mod == SDL_KMOD_RGUI) {
					extern short editstatus;
					if (editstatus) {
						ev.key.key = SDLK_INSERT;
						ev.key.scancode = SDL_SCANCODE_INSERT;
						ev.key.mod &= ~SDL_KMOD_RGUI;
					}
				}
#endif
				code = keytranslation[ev.key.scancode].normal;
				control = keytranslation[ev.key.scancode].controlchar;

				if (control && ev.key.down) {
					int needcontrol = (control & WITH_CONTROL_KEY) == WITH_CONTROL_KEY;
					int mod = ev.key.mod & ~(SDL_KMOD_CAPS|SDL_KMOD_NUM);
					...
					if ((needcontrol  && mod && (mod & SDL_KMOD_CTRL) == mod) ||
						(!needcontrol && (mod == SDL_KMOD_NONE))) {
				...
				if (code == OSD_CaptureKey(-1)) {
					...
					break;
				} else if (OSD_HandleKey(code, ev.key.down) == 0)
					break;
			...
			case SDL_EVENT_WINDOW_FOCUS_LOST:
				appactive = ev.window.type == SDL_EVENT_WINDOW_FOCUS_GAINED;
				if (mouseacquired && moustat) {
					SDL_SetWindowRelativeMouseMode(sdl_window, !!appactive);
				}
				rv=-1;
				break;
			...
			case SDL_EVENT_GAMEPAD_AXIS_MOTION:
				if (appactive) {
					joyaxis[ ev.gaxis.axis ] = ev.gaxis.value;
				}
				break;
...
static int set_glswapinterval(const osdfuncparm_t *parm)
{
	...
	if (!SDL_GL_SetSwapInterval(interval)) {
		buildprintf("error: could not change swap interval: %s\n", SDL_GetError());
		 ...
		 SDL_GL_GetSwapInterval(&glswapinterval);
	}
	return OSDCMD_OK;
}
#endif
